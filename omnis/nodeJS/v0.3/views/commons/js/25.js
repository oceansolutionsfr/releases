 ...
 */
UI.RenderDropZone = function(name, onaddedfile) {
    $A(name).addClass('a-dropzone');
    $('#' + name).append("<form action='#' id='dz" + name + "' class='dropzone dz-clickable'><div class='dz-message' data-dz-message id='mmes-" + name + "'></div></form>");
    Dropzone.options["dz" + name] = {
        autoProcessQueue: false,
        thumbnailWidth: 10,
        thumbnailHeight: 10,
        init: function() {
            this.on("addedfile", function(file) {
                onaddedfile(file, name);
                this.removeAllFiles();
